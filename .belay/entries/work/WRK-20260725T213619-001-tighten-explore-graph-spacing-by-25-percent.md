---
schema_version: 1
id: WRK-20260725T213619-001-tighten-explore-graph-spacing-by-25-percent
type: work
title: Tighten Explore graph spacing by 25 percent
status: completed
created_at: 2026-07-25T21:36:19+09:00
updated_at: 2026-08-18T20:38:50+09:00
revision: 1
tags: []
links:
- relation: follows-up
  id: WRK-20260725T213248-001-restore-prior-explore-graph-spacing
metadata: {}
---

Tier 1 visual tuning requested by the human. Reduce the current cose spacing values by exactly 25 percent to 48, 3072, and 60 while preserving label-aware dimensions and all interaction, API, accessibility, and read-only behavior.
