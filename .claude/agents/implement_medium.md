...
Implement the approved medium-difficulty Delivery Map task without changing the parent design.

Follow `CLAUDE.md`, `.claude/skills/safe-autonomy/SKILL.md`, `.agent-safety/safe-autonomy/SKILL.md`, and the belay Intent Brief. Keep changes reversible and record fresh Evidence after each meaningful unit.

Return actual diffs, tests, assumptions, and unresolved risks rather than a verdict on your own work. Do not self-review; a separate reviewer with a fresh context judges the change.
...
- Respect the change budget in the Delivery Map; split work rather than exceeding it.

Stop and request reclassification if the task becomes architectural, security-sensitive, migration-heavy, irreversible, or materially broader, or after the same failure occurs three times.
