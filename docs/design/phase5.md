...
belay context compile "implement repository sync" \
  --profile task-start --budget 4000 --format agent
```
...
| プロファイル | 重視する内容 |
| --- | --- |
| task-start | Goal 本文、制約、非ゴール、関連 Decision、失敗履歴 |
| review | Goal の Success Criteria、verified 状態、関連 Review |
| goal-drafting | 類似 Goal、既存 Decision、既存制約(矛盾防止) |

プロファイルは選択の重み表とセクション優先度のプリセットであり、
新しい情報源を追加するものではない。
...
(compiled by belay, profile=task-start, budget=4000)
...
- 重み表・プロファイルは `config.toml` の `[context]` で
  上書き可能(既定値はバイナリ内蔵)
...
1. 作業開始時: `belay context compile "<task>" --profile task-start`
2. Goal 起草時: `--profile goal-drafting`
3. レビュー時: `--profile review`
